...
import (
	...
	"github.com/LDZ6/iam/internal/apiserver/store/fake"
)
...
func (s *Suite) Test_userService_DeleteCollection() {
	s.mockUserStore.EXPECT().DeleteCollection(gomock.Any(), []string{"admin", "john"}, gomock.Any()).Return(nil)
...
func (s *Suite) Test_userService_Delete() {
	s.mockUserStore.EXPECT().Delete(gomock.Any(), "admin", gomock.Any()).Return(nil)
...
func (s *Suite) Test_userService_Get() {
	s.mockUserStore.EXPECT().Get(gomock.Any(), "admin", gomock.Any()).Return(s.users[0], nil)
