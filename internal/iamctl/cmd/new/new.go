...
import (
	...
	"github.com/LDZ6/component-base/pkg/util/fileutil"
...
to quickly create a Cobra application.{{.Dot}})
	...
	{{.CommandName}}Example = templates.Examples({{.Dot}}
		...
		iamctl {{.CommandName}} LDZ6 LDZ6pass{{.Dot}})
...
to quickly create a Cobra application.{{.Dot}})
	...
	subcmd1Example = templates.Examples({{.Dot}}
		...
		iamctl {{.CommandName}} subcmd1 LDZ6 LDZ6pass

		# Print all option values for subcmd1 with --persistent specified
		iamctl {{.CommandName}} subcmd1 LDZ6 LDZ6pass --persistent="specified persistent option in command line"{{.Dot}})
