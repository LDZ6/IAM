...
var errCodeDocPrefix = `# 错误�?
！！IAM 系统错误码列表，�?{{.}}codegen -type=int -doc{{.}} 命令生成，不要对此文件做任何更改�?
## 功能说明

如果返回结果中存�?{{.}}code{{.}} 字段，则表示调用 API 接口失败。例如：
...
上述返回�?{{.}}code{{.}} 表示错误码，{{.}}message{{.}} 表示该错误的具体信息。每个错误同时也对应一�?HTTP 状态码，比如上述错误码对应�?HTTP 状态码 500(Internal Server Error)�?
## 错误码列�?
IAM 系统支持的错误码列表如下�?
...
func (g *Generator) generate(typeName string) {
	...
	g.Printf("\t// init register error codes defines in this source code to `github.com/LDZ6/errors`\n")
	g.Printf("func init() {\n")
