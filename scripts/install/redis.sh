...
function iam::redis::install()
{
  ...
  # 2. 配置 Redis
  # 2.1 修改 `/etc/redis.conf` 文件，将 daemonize �?no 改成 yes，表示允�?Redis 在后台启�?  echo ${LINUX_PASSWORD} | sudo -S sed -i '/^daemonize/{s/no/yes/}' /etc/redis.conf
  ...
  # 2.4 因为我们上面配置了密码登录，需要将 protected-mode 设置�?no，关闭保护模�?  echo ${LINUX_PASSWORD} | sudo -S sed -i '/^protected-mode/{s/yes/no/}' /etc/redis.conf

  # 3. 为了能够远程连上 Redis，需要执行以下命令关闭防火墙，并禁止防火墙开机启动（如果不需要远程连接，可忽略此步骤�?  iam::common::sudo "systemctl stop firewalld.service"
